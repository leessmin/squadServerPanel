...
// 监听配置 switch按钮才需要监听，别的都不需要
watch([
    () => ServerConfig.ShouldAdvertise,
    () => ServerConfig.IsLANMatch,
    () => { ServerConfig.MapRotationMode },
    () => { ServerConfig.RandomizeAtStart },
    () => { ServerConfig.UseVoteFactions },
    () => { ServerConfig.UseVoteLevel }
],
    ([ShouldAdvertise, IsLANMatch, MapRotationMode, RandomizeAtStart, UseVoteFactions]) => {
        console.log(ShouldAdvertise);
    })
</script>
