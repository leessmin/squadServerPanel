<template>
	<!-- 修改账号密码 弹出框 -->
	<!-- 修改账号密码 -->
	<a-modal v-model:visible="isVisible" title="修改账号密码" @ok="userHandleOk" :footer="null">
...
const handleFinish = (values: FormState) => {
	...
	let newUser = values.user
	// 判断是否填写新用户名
	if (newUser == "") {
		newUser = values.oldUser
	}

	// 发送请求
	settingStore.editAdmin(values.oldUser, values.oldPass, newUser, values.checkPass)
};
