<template>
	<div class="btn-box">
		...
		<a-button type="primary" danger @click="cancelHandle">取消</a-button>
	</div>
	<div class="map">
		<div class="left">
			<h3>已选择的地图</h3>
...
const cancelHandle = async () => {
	...
	console.log(JSON.parse(JSON.stringify(props.mapArr.get("selected"))));
	// 刷新dom  防止v-for没有刷新
	await nextTick()
	mapSelected.value = JSON.parse(JSON.stringify(props.mapArr.get("selected")))
	mapSelect.value = JSON.parse(JSON.stringify(props.mapArr.get("select")))
}
...
function isCfgChange(): boolean {
	...
	return !(JSON.stringify(getUlMapValue(mapUlDom.value)) == JSON.stringify(props.mapArr.get("selected")))
}
</script>
