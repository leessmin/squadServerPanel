...
// 保存 回调
const save = () => {
	squadStore.saveSquadMsg(messageList.value)
}

// 取消 回调
const cancel =()=>{
	squadStore.getSquadMsg()
}
...
squadStore.$subscribe((mutation, state) => {
	messageList.value = JSON.parse(JSON.stringify(state.serverMsg))
})
...
function isCfgChange(): boolean {
	...
	return !(JSON.stringify(squadStore.serverMsg) == JSON.stringify(messageList.value))
}
</script>
